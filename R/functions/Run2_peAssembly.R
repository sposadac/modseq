...
Run2_peAssembly <- function(readF1, readF2, wdir, out.filename.run2, len.rawData,
                            qtrim.flag, out.ssplot, forward.file=NULL, 
                            reverse.file=NULL, in.seqDir=NULL) { 
  ...
  # -F         Output FASTQ.
  # -w file    Output seqences to a FASTA.
  ...
  if (qtrim.flag == 0) { 
    ...
    cat("Forward-reads file: \"", in.seqDir, forward.file, ".fastq\"\n",
        sep = "")
    cat("Reverse-reads file: \"", in.seqDir, reverse.file, ".fastq\"\n", 
        sep = "")
    assign(paste("runPANDAseq_", out.filename.run2, sep = ""), 
           paste("pandaseq -f ", in.seqDir, forward.file, ".fastq -r ", 
                 in.seqDir, reverse.file, ".fastq -C qualString:data/paired", 
    ...
    cat("Forward-reads file: \"", forward.file, "\"\n", sep = "")
    cat("Reverse-reads file: \"", reverse.file, "\"\n", sep = "")
    #assign(paste("runPANDAseq_", out.filename.run2, sep = ""),
    #       paste("pandaseq -f ", forward.file, "-r ", reverse.file, 
    #             "-C qualString:data/paired", out.filename.run2, "_PANDAseq",
    #             ".fastq -B -F -w data/paired/fasta/", out.filename.run2,
    #             "_PANDAseq.fasta -g data/paired/logPANDAseq_",
    #             out.filename.run2, ".txt -U data/paired/", out.filename.run2,
    #             "_unalignedPANDAseq.fastq", sep = ""))
    assign(paste("runPANDAseq_", out.filename.run2, sep = ""),
           paste("pandaseq -f ", forward.file, "-r ", reverse.file, 
                 "-B -F data/paired", out.filename.run2, "_PANDAseq",
                 ".fastq -w data/paired/fasta/", out.filename.run2,
                 "_PANDAseq.fasta -g data/paired/logPANDAseq_",
                 out.filename.run2, ".txt -U data/paired/", out.filename.run2,
                 "_unalignedPANDAseq.fastq", sep = ""))
  ...
  cat("Paired reads: ", len.peData, " of ", len.rawData, " (",
    ...
    if (qtrim.flag == 0) {
      ...
      if (file.exists(
        file.path(wdir, 'data/processed', 
                  paste(out.filename.run1, "_subSet_wo0_1_nQtrim", sep = "")))) {
        IlluminaStat(in.dir = file.path(wdir, 'data/paired/'), 
                     pattern = out.filename.run2, SV_plotinputframe = plot.input,
                     SV_qcwd1 = avQScore.readF1, SV_qcwd2 = avQScore.readPE, 
                     SV_meanread1 = mean.readF1, SV_meanread2 = mean.readF2, 
        ...
        #**TODO**
        #check if the forward.file doesn't have reads with length zero 
        IlluminaStat(in.dir = file.path(wdir, 'data/paired/'), 
                     pattern = out.filename.run2, SV_plotinputframe = plot.input,
                     SV_qcwd1 = avQScore.readF1, SV_qcwd2 = avQScore.readPE, 
                     SV_meanread1 = mean.readF1, SV_meanread2 = mean.readF2, 
      ...
      #**TODO**
      # Search for "_subSet_wo0_1" in the in.seqDir
