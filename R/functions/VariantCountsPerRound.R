VariantCountsPerRound <- function(patterns, num.reads, counts, labels, 
                                  file.prefix, out.dir, gls.direction = "f",
  ...
  if (!is.data.frame(patterns)) {
    ...
    if (is.character(patterns)) {
      patterns <- LoadModuleTable(in.modulesDir = patterns, list = FALSE) 
    } else {
